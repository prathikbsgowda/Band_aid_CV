...
def main():
    ...
    if arm_points is not None:
        if len(arm_points) == 3:  # Mediapipe (shoulder, elbow, wrist)
            for pt in arm_points:
                cv2.circle(debug, pt, 5, (0,255,0), -1)  # green dots
            cv2.line(debug, arm_points[1], arm_points[2], (255,0,0), 2)  # elbow-wrist line
        else:  # Skin-based (pt1, center, pt2)
            cv2.circle(debug, arm_points[1], 5, (0,0,255), -1)  # center red
            cv2.line(debug, arm_points[0], arm_points[2], (0,255,255), 2)  # axis line
    cv2.imwrite("results/debug.jpg", debug)
    print("Saved debug visualization to results/debug.jpg")
